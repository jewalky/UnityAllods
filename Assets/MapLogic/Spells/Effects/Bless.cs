...
{
    class Bless : TimedEffect
    {
        bool Attached = false;
        int Power = 0;

        public Bless(int duration, int power) : base(duration)
        {
            Attached = false;
            Power = power;
        }
        ...
        {
            stats.Bless += (byte)Power;
        }
    }

    [SpellIndicatorFlags(UnitFlags.Bless)]
    public class BlessIndicator : EffectIndicator
    {
        public BlessIndicator(MapUnit unit) : base(unit) { }
        ...
        public override void OnEnable()
        {
            ...
            {
                MapProjectile item = new MapProjectile(AllodsProjectile.Bless, Unit);
